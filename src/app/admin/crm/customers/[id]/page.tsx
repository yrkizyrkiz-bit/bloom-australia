              ...
              {subscription ? (
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="p-4 bg-white rounded-xl border">
                          ...
                          <p className="text-lg font-bold">${subscription.amount} <span className="text-sm font-normal text-muted-foreground">/{subscription.billingCycle === "monthly" ? "mo" : subscription.billingCycle === "yearly" ? "yr" : subscription.billingCycle}</span></p>
                        </div>
                        <div className="p-4 bg-white rounded-xl border">
                        ...
                        <div className="p-4 bg-white rounded-xl border">
                          ...
                          <p className="text-lg font-bold capitalize">{subscription.billingCycle}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-medium">{subscription.startDate ? new Date(subscription.startDate).toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" }) : "—"}</p>
                        </div>
                        <div>
                          <Label className="text-muted-foreground flex items-center gap-1">
                          ...
                          <p className="font-medium">{subscription.currentPeriodEnd ? new Date(subscription.currentPeriodEnd).toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" }) : "—"}</p>
                        </div>
                      </div>
                ...
                <Card>
                  <CardContent className="py-12 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">This customer does not have an active subscription plan.</p>
              ...
              <Card>
                ...
                <CardContent>
                  {bookings.length > 0 ? bookings.map((b: Record<string, unknown>) => (
                    <div key={b.id as string} className="flex justify-between p-3 border rounded-lg mb-2">
                      <div>
                        <p className="font-medium">{new Date(b.scheduledAt as string).toLocaleDateString("en-AU", { weekday: "long", day: "numeric", month: "long" })}</p>
                        <p className="text-xs text-muted-foreground">{new Date(b.scheduledAt as string).toLocaleTimeString("en-AU", { hour: "numeric", minute: "2-digit" })}{b.doctorName ? ` • ${b.doctorName as string}` : ""}</p>
                      </div>
                      <Badge>{b.status as string}</Badge>
                    </div>
