...
export default function CRMDashboardPage() {
  ...
  const memberStats = useMemo(() => ({
    ...
    potential: members.filter((m) => m.memberStatus === "POTENTIAL_MEMBER").length,
    activeMembers: members.filter((m) => m.memberStatus === "MEMBER").length,
    cancelled: members.filter((m) => m.memberStatus === "CANCELLED").length,
    ...
    const fetchMembers = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setMembers(
            (data.users || []).map((user: MemberRecord) => ({
              ...user,
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage members, billing, and communications</p>
        </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-serif font-bold text-foreground mt-1">
                    {membersLoading ? "—" : memberStats.total}
                  </p>
                  <p className="text-xs text-green-600 mt-1 flex items-center gap-1">
                    <TrendingUp className="w-3 h-3" />
                    +{membersLoading ? "—" : memberStats.newThisMonth} this month
                  </p>
                </div>
        ...
        <Link href="/admin/crm/customers?memberStatus=POTENTIAL_MEMBER">
          <Card className="hover:border-amber-500/50 border-amber-200 bg-amber-50/20 transition-colors cursor-pointer h-full">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-amber-700">Potential Members</p>
                  <p className="text-3xl font-serif font-bold text-amber-600 mt-1">
                    {membersLoading ? "—" : memberStats.potential}
                  </p>
                  <p className="text-xs text-amber-700 mt-1">Quiz started, not yet paid</p>
                </div>
        ...
        <Link href="/admin/crm/customers?memberStatus=MEMBER">
          <Card className="hover:border-green-500/50 transition-colors cursor-pointer h-full">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">{MEMBER_STATUS_LABELS.MEMBER}s</p>
                  <p className="text-3xl font-serif font-bold text-green-600 mt-1">
                    {membersLoading ? "—" : memberStats.activeMembers}
                  </p>
        ...
        <Link href="/admin/crm/customers?memberStatus=CANCELLED">
          <Card className="hover:border-red-500/50 transition-colors cursor-pointer h-full">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">{MEMBER_STATUS_LABELS.CANCELLED}</p>
                  <p className="text-3xl font-serif font-bold text-red-600 mt-1">
                    {membersLoading ? "—" : memberStats.cancelled}
                  </p>
      ...
      {/* Revenue snapshot (legacy mock data) */}
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-serif font-bold text-green-600 mt-1">${mockStats.monthlyRecurringRevenue}</p>
                <p className="text-xs text-muted-foreground mt-1">MRR (sample data)</p>
              </div>
              <div className="w-12 h-12 rounded-xl bg-green-500/10 flex items-center justify-center">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-serif font-bold text-foreground mt-1">${Math.round(mockStats.averageLifetimeValue)}</p>
                <p className="text-xs text-muted-foreground mt-1">per member (sample data)</p>
              </div>
          ...
          <Card className="hover:border-primary/50 transition-colors cursor-pointer">
            <CardContent className="pt-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{mockStats.openTasks} open</p>
              </div>
            </CardContent>
          </Card>
        </Link>
