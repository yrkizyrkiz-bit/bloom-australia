...
const PHASE_FOCUS: Record<string, string[]> = {
  INDUCTION: ["getting_started", "medication"],
  TITRATION: ["nutrition", "exercise", "medication"],
  MAINTENANCE: ["nutrition", "exercise"],
};

export default function LearnPage() {
  const [activeCategory, setActiveCategory] = useState("all");
  const [programPhase, setProgramPhase] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/program/today")
      .then((r) => r.json())
      .then((d) => {
        if (d.hasProgram && d.program?.phase) {
          setProgramPhase(d.program.phase);
          const focus = PHASE_FOCUS[d.program.phase];
          if (focus?.length) setActiveCategory(focus[0]);
        }
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6">
      ...
      {programPhase && (
        <Card className="border-emerald-200 bg-emerald-50/80 dark:bg-emerald-950/20">
          <CardContent className="p-4 text-sm">
            <span className="font-semibold text-emerald-800 capitalize">
              {programPhase.toLowerCase()} phase
            </span>
