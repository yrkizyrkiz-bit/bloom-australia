...
import {
  ORGAN_CARE_AI_ANALYSIS_TYPE,
  generateOrganCareReport,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const context = await loadOrganCareReportContext(userId);
    ...
    const cachedReport = await prisma.aIAnalysisCache.findUnique({
      where: {
        userId_analysisType: { userId, analysisType: ORGAN_CARE_AI_ANALYSIS_TYPE },
      },
    });
    ...
    return NextResponse.json({ error: "Failed to load Organ Care AI report" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!context.biomarkerHash || context.biomarkerCount === 0) {
      return NextResponse.json(
        { error: "No organ or metabolic biomarker results found", requiresBiomarkers: true },
        { status: 400 }
      );
    }
    ...
    if (existingReport && existingReport.biomarkerHash === context.biomarkerHash) {
      return NextResponse.json(
        {
          report: normalizeOrganCareReport(
          ...
          blocked: true,
          reason:
            "A report already exists for the latest blood test. Upload new results to generate another report.",
          dataDate: context.dataDate,
          resultsStale: context.resultsStale,
        },
        { status: 409 }
    ...
    const { report, usedFallback } = await generateOrganCareReport(context);
    ...
    console.error("[organ-care/ai-report] POST", error);
    return NextResponse.json({ error: "Failed to generate Organ Care AI report" }, { status: 500 });
  }
}
