...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !session?.user?.id ||
      !["ADMIN", "CARE_PARTNER"].includes(session.user.role || "")
    ) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
    ...
    // Mark onboarding/welcome call as completed; optional manual program activation
    if (action === "completeOnboarding") {
      const onboardingNotes: string | null = data.notes || null;
      const activateProgram = data.activateProgram === true;

      const onboardingTask = await prisma.careCommunication.findFirst({
        where: { userId: memberId, type: "ONBOARDING" },
        orderBy: { createdAt: "desc" },
      ...
      const mergedNotes =
        onboardingNotes && onboardingNotes.trim().length > 0
          ? `${onboardingTask.notes || ""}\n\nCare partner onboarding notes: ${onboardingNotes}`
          : onboardingTask.notes;

      const updatedTask = await prisma.careCommunication.update({
      ...
      if (activateProgram) {
        const { activateMemberProgram } = await import(
          "@/lib/program/activate-member-program"
        );
        const activation = await activateMemberProgram(
          memberId,
          session.user.id
        );
        return NextResponse.json({
          success: true,
          onboarding: updatedTask,
          activation,
        });
      }
      ...
      await prisma.user.update({
        where: { id: memberId },
        data: { journeyStatus: "ONBOARDING_COMPLETE" },
      });

      return NextResponse.json({ success: true, onboarding: updatedTask });
    }

    // Standalone manual activation (e.g. welcome call already completed)
    if (action === "activateProgram") {
      const { activateMemberProgram } = await import(
        "@/lib/program/activate-member-program"
      );
      const activation = await activateMemberProgram(
    ...
    // Create a care task to upsell billing to quarterly / 6-month
    if (action === "createBillingUpgradeOffer") {
      const created = await prisma.careCommunication.create({
        data: {
          userId: memberId,
          type: "BILLING_UPGRADE_OFFER",
          priority: "NORMAL",
          subject: `Quarterly / 6-month billing upgrade offer: ${member.firstName} ${member.lastName}`,
          notes:
            "Care partner: discuss billing upgrade options (quarterly / 6-month) during the welcome call or subsequent follow-up. Subscription conversion may require manual Stripe setup depending on the configured pricing.",
          status: "PENDING",
