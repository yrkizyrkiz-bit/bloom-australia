...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (existing) {
      // User exists — update with any new data provided
      ...
      if (data.streetAddress?.trim()) updateData.addressLine1 = data.streetAddress.trim();
      if (data.addressUnit?.trim()) updateData.addressLine2 = data.addressUnit.trim();
      if (data.suburb?.trim()) updateData.suburb = data.suburb.trim();
      if (data.state?.trim()) updateData.state = data.state.trim();
      if (data.postcode?.trim()) updateData.postcode = data.postcode.trim();
      if (data.dateOfBirth) updateData.dateOfBirth = parseDOB(data.dateOfBirth);
      if (data.gender) updateData.gender = mapGender(data.gender);
      if (data.howHeard) updateData.leadSource = data.howHeard;

      // Update journey status if progressing
      if (data.journeyStatus) {
        updateData.journeyStatus = data.journeyStatus;
      } else if (data.streetAddress) {
        // They've provided address = completed shipping step
        updateData.journeyStatus = "SURVEY_COMPLETED";
      }
      ...
      if (Object.keys(updateData).length > 0) {
        ...
        console.log("[Intake API] Updated existing user:", existing.id, updateData);
      }

      // ─── CRITICAL: Create or update WeightManagementIntake ───────────────────
      let intakeId: string | null = null;
      let programMemberId: string | null = null;
      let bookingId: string | null = null;

      // ─── Update ProgramMember.intakeData with ALL quiz data ───────
      // Merge new data with existing intakeData to capture full quiz progression
      try {
        ...
        if (existingMember) {
          // Merge existing intakeData with new data (new data overwrites)
          const existingIntakeData = (existingMember.intakeData as Record<string, unknown>) || {};
          const mergedIntakeData = {
            ...existingIntakeData,
            ...data, // New data overwrites existing
            // Ensure we always have timestamps
            updatedAt: new Date().toISOString(),
            firstSubmittedAt: existingIntakeData.firstSubmittedAt || existingIntakeData.submittedAt || new Date().toISOString(),
          };
          ...
          await prisma.programMember.update({
            ...
            data: {
              ...
              ...(data.phone?.trim() && { mobile: data.phone.trim() }),
              ...(data.dateOfBirth && { dob: parseDOB(data.dateOfBirth) }),
            }
          });
          programMemberId = existingMember.id;

          console.log("[Intake API] Updated ProgramMember intakeData:", existingMember.id, {
            fieldsUpdated: Object.keys(data).length,
            totalFields: Object.keys(mergedIntakeData).length,
          });
        } else {
          // Create ProgramMember if it doesn't exist (edge case)
          const membershipEnd = new Date();
          membershipEnd.setFullYear(membershipEnd.getFullYear() + 1);

          // GAP-008: Set membership as PENDING during intake, not ACTIVE
          // Patient can only become ACTIVE after all activation criteria are met
          const newMember = await prisma.programMember.create({
            data: {
              userId:           existing.id,
              firstName:        data.firstName?.trim() || existing.firstName || "",
              lastName:         data.lastName?.trim() || existing.lastName || "",
              email:            data.email?.toLowerCase().trim(),
              mobile:           data.phone?.trim() || existing.phone || "",
              dob:              data.dateOfBirth ? parseDOB(data.dateOfBirth) : (existing.dateOfBirth || new Date()),
              program:          programType,
              intakeData:       { ...data, submittedAt: new Date().toISOString() },
              membershipStatus: "PENDING", // GAP-008: Not ACTIVE until activation criteria met
              membershipStart:  new Date(),
              membershipEnd:    membershipEnd,
            }
          });
          programMemberId = newMember.id;
          console.log("[Intake API] Created ProgramMember for existing user (PENDING):", existing.id);
        }
      } catch (memberError) {
        console.error("[Intake API] Error updating ProgramMember:", memberError);
      }

      // ─── Create or find ConsultationBooking ───────────────────────────────────
      if (programType === "WEIGHT_MANAGEMENT" && data.consultationDate && data.consultationTime) {
        try {
          ...
          if (existingBooking) {
            ...
            console.log("[Intake API] Found existing booking:", bookingId);
          } else {
            ...
            console.log("[Intake API] Created ConsultationBooking for existing user:", bookingId);
          }
        } catch (bookingError) {
          console.error("[Intake API] Error with ConsultationBooking:", bookingError);
        }
      }

      // ─── Create or update WeightManagementIntake (with full linking) ──────────
      if (programType === "WEIGHT_MANAGEMENT") {
        try {
          ...
          if (existingIntake) {
            ...
            const mergedQuizData = { ...existingQuizData, ...data, updatedAt: new Date().toISOString() };

            const updatedIntake = await prisma.weightManagementIntake.update({
              where: { id: existingIntake.id },
              data: {
                quizData: mergedQuizData,
                selectedPlan: data.selectedPlan === "precision" ? "PRECISION" : "CORE",
                portalStatus: data.streetAddress ? "INTAKE_COMPLETED" : existingIntake.portalStatus,
                completedAt: data.streetAddress ? new Date() : existingIntake.completedAt,
                // Link to ProgramMember if not already linked
                ...(programMemberId && !existingIntake.programMemberId && { programMemberId }),
                // Link to booking if not already linked
                ...(bookingId && !existingIntake.bookingId && {
                  bookingId,
                  bookingStatus: "SLOT_HELD",
                }),
              },
            });
            intakeId = updatedIntake.id;
            console.log("[Intake API] Updated WeightManagementIntake with links:", {
              intakeId,
              programMemberId: updatedIntake.programMemberId,
              bookingId: updatedIntake.bookingId,
            });
          } else {
          ...
          console.error("[Intake API] Error with WeightManagementIntake:", intakeError);
        }
      }

      // Check if they're adding a new program
      if (existing.subscriptionTier !== config.subscriptionTier) {
        await prisma.internalNote.create({
          data: {
            userId:    existing.id,
            category:  "GENERAL",
            title:     `New program inquiry: ${programType}`,
            content:   `Patient submitted assessment for ${programType}. Consider cross-sell.`,
            createdBy: "system",
          }
        }).catch(console.error);
      }
