...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const activeBooking =
      (bookingId
        ? user.consultationBookings.find((b) => b.id === bookingId)
        : null) ||
      user.consultationBookings.find((b) =>
        ["SLOT_HELD", "BOOKING_CONFIRMED"].includes(b.status)
      ) ||
    ...
    if (activeBooking) {
      const hasScheduleNote = user.internalNotes.some(
        (n) => n.title === "Consultation Schedule"
      );
      if (!hasScheduleNote) {
        await syncConsultationScheduleNoteForBooking(activeBooking.id);
        const scheduleNote = await prisma.internalNote.findFirst({
        ...
        if (scheduleNote) {
          user.internalNotes.unshift(scheduleNote);
        }
      }
    }

    const scheduleHistory = activeBooking
      ? (await getScheduleHistoryForBooking(activeBooking.id)).map(formatScheduleHistoryEntry)
      : [];
    ...
    const patientBrief = {
      ...
      // Onboarding / welcome call task status (care partner)
      onboardingTask: onboardingTask
        ? {
            id: onboardingTask.id,
            status: onboardingTask.status,
            dueDate: onboardingTask.dueDate?.toISOString() || null,
            completedAt: onboardingTask.completedAt?.toISOString() || null,
          }
        : null,
      ...
      activeConsultation: activeBooking
        ? {
            id: activeBooking.id,
            scheduledAt: activeBooking.scheduledAt.toISOString(),
            status: activeBooking.status,
            doctorName: activeBooking.doctorName,
            selectedPlan: activeBooking.selectedPlan,
            patientBmi: activeBooking.patientBmi,
            riskFlags: activeBooking.riskFlags,
            scheduleHistory,
          }
        : null,
      ...
      clinicalNotes: user.internalNotes
        .filter((note) => !isSystemClinicalNote(note.title))
        .sort((a, b) => {
          const priority = (title: string) =>
            title === "Consultation Schedule" ? 0 : title === "Patient Brief for Doctor Review" ? 1 : 2;
          const diff = priority(a.title) - priority(b.title);
