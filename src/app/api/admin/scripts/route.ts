  ...
  try {
    ...
    if (newJourneyStatus) {
      await prisma.user.update({
        ...
        data: { journeyStatus: newJourneyStatus as "APPROVED" | "SCRIPT_WRITTEN" | "PHARMACY_PENDING" | "DISPENSING" | "SHIPPED" | "DELIVERED" },
      });
    }
...
4. Update status to DELIVERED`,
      ...
      case "DELIVERED": {
        ...
        // Check if patient meets activation criteria
        // Patient becomes ACTIVE after:
        // - Doctor approval complete ✓
        // - Prescription/script created ✓
        // - Onboarding complete (check)
        // - Medication dispatch confirmed (now confirmed) ✓
        // - No unresolved pathology review
        ...
        const onboardingComplete = await prisma.careCommunication.findFirst({
          where: {
            userId: prescription.patientId,
            type: "ONBOARDING",
            status: "COMPLETED",
        ...
        const canActivate = !pendingPathology && (onboardingComplete !== null);
        ...
        if (canActivate) {
          ...
          await sendEmail({
            ...
            body: `
              <h2>Hi ${user.firstName},</h2>
              <p>Your delivery has been confirmed and your Sanative program is now fully active!</p>
              ...
              <ul>
                <li>Log your starting weight in the app</li>
                <li>Set your goals and milestones</li>
                <li>Track your meals and activities</li>
                <li>Your care partner is always available for support</li>
              </ul>
        ...
        // Create welcome call task if not already done
        await prisma.careCommunication.create({
          data: {
            userId: prescription.patientId,
            type: "CALL_SCHEDULED",
            priority: "HIGH",
            subject: `Welcome Call (Post-Delivery): ${user.firstName} ${user.lastName}`,
            notes: `Medication has been delivered.

**Call Topics:**
1. Confirm delivery received and checked
2. Review storage and usage instructions
3. Answer any questions
4. Set up first check-in appointment
5. Discuss expectations for first month

${!canActivate && pendingPathology ? "Note: Patient has pending pathology - do not activate subscription until resolved." : ""}`,
            status: "PENDING",
            dueDate: new Date(Date.now() + 24 * 60 * 60 * 1000),
            assignedTo: user.assignedCarePartnerId || undefined,
          },
