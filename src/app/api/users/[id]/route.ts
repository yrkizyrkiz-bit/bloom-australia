...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!canChangeRoleOrSubscription && (role || subscriptionStatus || subscriptionTier || memberStatus)) {
    ...
    if (state !== undefined || postcode !== undefined) {
      const existing = await prisma.user.findUnique({
        where: { id },
        select: { state: true, postcode: true },
      });
      updateData.timezone = resolveAustralianTimezone(
        (state as string | undefined) ?? existing?.state,
        (postcode as string | undefined) ?? existing?.postcode
      );
    }
    ...
    if (canChangeRoleOrSubscription) {
      ...
      if (memberStatus !== undefined) updateData.memberStatus = memberStatus.toUpperCase();
    }
    ...
    const user = await prisma.user.update({
      ...
      select: {
        ...
        // Residential address
        address: true, addressLine1: true, addressLine2: true, suburb: true, state: true, postcode: true, country: true, timezone: true,
        // Mailing address
        mailingAddressSameAsResidential: true, mailingAddress: true, mailingAddressLine1: true, mailingAddressLine2: true,
        mailingSuburb: true, mailingState: true, mailingPostcode: true, mailingCountry: true,
        // Other
