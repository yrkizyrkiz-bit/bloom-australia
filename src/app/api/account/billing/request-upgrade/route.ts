...
import { getMemberBillingOverview } from "@/lib/billing/member-billing-summary";
...
export async function POST() {
  try {
    ...
    const overview = await getMemberBillingOverview(userId);
    const summary =
      overview.programs.find((p) => p.program === "weight_management") ??
      overview.programs[0];
