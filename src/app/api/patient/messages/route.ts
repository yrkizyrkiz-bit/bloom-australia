...
import { ensureWMProgramMember } from "@/lib/wm/ensure-program-member";
...
export async function GET() {
  try {
    ...
    let member = await prisma.programMember.findUnique({
    ...
    if (!member && session.user.id && (await isWeightManagementUser(session.user.id))) {
      await ensureWMProgramMember(session.user.id);
      member = await prisma.programMember.findUnique({
        where: { email: session.user.email },
        include: {
          carePartner: {
