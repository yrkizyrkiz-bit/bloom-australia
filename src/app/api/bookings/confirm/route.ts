...
const MAGIC_LINK_SECRET = process.env.MAGIC_LINK_SECRET || process.env.NEXTAUTH_SECRET || "fallback-secret-change-me";
...
• Date: ${scheduledAt.toLocaleString("en-AU", { timeZone: TIMEZONE })}
...
• Scheduled: ${scheduledAt.toLocaleString("en-AU", { timeZone: TIMEZONE, weekday: "long", day: "numeric", month: "long", hour: "numeric", minute: "2-digit" })}
...
function generateMagicLink(userId: string, email: string, requestUrl?: string): string {
  const token = sign(
  ...
  // Determine base URL: use request origin if available, otherwise env vars
  let baseUrl = "https://sanative.com.au"; // Production default
  ...
  // Override with env vars if they look like production URLs (not localhost)
...
async function sendConfirmationEmail(
  ...
  data: {
    ...
    doctorName: string;
    selectedPlan: string;
    magicLink?: string; // Magic link for portal access
  }
): Promise<void> {
  const formattedDate = data.scheduledAt.toLocaleString("en-AU", {
    timeZone: TIMEZONE,
    weekday: "long",
    day: "numeric",
    month: "long",
  ...
  try {
    await sendEmail({
      to: email,
      subject: `Your Sanative consultation is confirmed for ${data.scheduledAt.toLocaleDateString("en-AU", { weekday: "long", day: "numeric", month: "long" })}`,
      body: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #f4f7f2; border-radius: 12px; padding: 24px; margin: 24px 0;">
            ...
            <table style="width: 100%; color: #5c7a52;">
              <tr>
                ...
                <td style="padding: 8px 0;">${formattedDate} (Sydney time)</td>
              </tr>
              <tr>
          ...
          <ol style="color: #5c7a52; line-height: 1.8;">
            ...
            <li>Your medication will be shipped in discreet packaging</li>
          </ol>
          ...
          <div style="margin: 32px 0;">
            <a href="${data.magicLink || `${baseUrl}/dashboard`}" style="display: inline-block; background: #5c7a52; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: 600;">
              Access My Portal
            </a>
          </div>

          <p style="color: #666; font-size: 14px;">
            Click the button above to set up your account and access your Weight Management dashboard.
...
async function sendConfirmationSMS(
  ...
  const message = `Hi ${data.firstName}, your Sanative consultation is confirmed for ${formattedDate} with ${data.doctorName}. We'll call you at this number. Reply STOP to opt out.`;
...
async function createPreTriageTask(
  ...
  try {
    ...
    // Also update user journey status
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { bookingHoldId, paymentIntentId, userId: bodyUserId, sessionId, selectedPlan } = body;
    ...
    let user: { id: string; email: string; firstName: string; lastName: string; phone: string | null } | null = null;
      ...
      updatedBooking = await prisma.$transaction(async (tx) => {
        ...
        if (bookingUserId) {
          await tx.user.update({
            where: { id: bookingUserId },
            data: { journeyStatus: "CONSULTATION_PAID" },
          });
        }
    ...
    if (user?.email && bookingUserId) {
      const magicLink = generateMagicLink(bookingUserId, user.email);
    ...
    // Format the response
    const formattedDate = booking.scheduledAt.toLocaleString("en-AU", {
      timeZone: TIMEZONE,
      weekday: "long",
      day: "numeric",
      month: "long",
    ...
    const portalMagicLink = bookingUserId && user?.email
      ? generateMagicLink(bookingUserId, user.email)
      : undefined;
