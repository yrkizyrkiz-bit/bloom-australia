...
interface PaymentFormProps {
  ...
  formId?: string;
  /** Defer payment intent until slot is held */
  enabled?: boolean;
  onReadyChange?: (ready: boolean) => void;
  onProcessingChange?: (processing: boolean) => void;
  /** Card number on top; expiry + CVC in a row below */
  splitCardFields?: boolean;
  /** Pay button inside the form (below card fields) */
...
function CheckoutForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const cardElement = splitCardFields
  ...
  const cardStyle = {
    ...
    // Hide postal code - not needed for Australian payments
    hidePostalCode: true,
  };
  ...
  return (
    <form id={formId} onSubmit={handleSubmit} className={embedded ? "space-y-3" : "space-y-5"}>
      {/* Card Input */}
      <div className={embedded ? "" : "bg-white rounded-2xl border border-[#e6ebe3] p-5"}>
        {!embedded && (
        ...
        {splitCardFields ? (
          <div className="space-y-2">
            <div>
              <label className="text-[10px] font-medium text-[#7e9a72] uppercase tracking-wide mb-1 block">
                Card number
              </label>
        ...
        >
          {isProcessing ? (
            ...
            <>
              ...
              {submitLabel ?? `Pay $${(amount / 100).toFixed(0)} & book consultation`}
            </>
          )}
        </button>
      )}
...
export function StripePaymentForm({
  ...
  onReadyChange,
  onProcessingChange,
  splitCardFields = false,
  showEmbeddedSubmit = false,
  submitLabel,
  ...
  const createPaymentIntent = useCallback(async () => {
    ...
    try {
      ...
      setPaymentIntentHoldId(bookingHoldId ?? null);
      setLoadError(null);
    } catch (err) {
