...
interface HistoryItem extends LiverAnalysisResult {
  id: string;
  overallScore: number;
  riskLevel: string;
...
export function PredictiveHealthRisk() {
  const [activeTab, setActiveTab] = useState("current");
  const [expandedRisk, setExpandedRisk] = useState<string | null>(null);
  const [analysis, setAnalysis] = useState<LiverAnalysisResult | null>(null);
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const response = await fetch("/api/liver-analysis/history");
      if (response.ok) {
  ...
  const fetchAnalysis = useCallback(async (forceRefresh = false) => {
    ...
    try {
      const url = forceRefresh ? "/api/liver-analysis?refresh=true" : "/api/liver-analysis";
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case "liver": return <Droplets className="w-4 h-4" />;
  ...
  // Initial loading state (only show when no analysis exists yet)
        ...
        <CardContent className="py-20">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="text-center">
              <h3 className="text-xl font-semibold text-slate-800 mb-2">Analyzing Your Liver Health</h3>
              <p className="text-slate-500 max-w-md">
                Claude AI is reviewing your biomarker data to generate personalized risk insights...
              </p>
            </div>
  ...
  // Error state (only show when no analysis exists)
  if (error && !analysis) {
    return (
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-2xl font-serif font-semibold text-slate-800">AI Health Risk Analysis</h2>
            ...
            <p className="text-slate-500 text-sm mt-0.5">
              Personalized liver health assessment based on your biomarkers
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        <Card className="border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50">
          <CardContent className="py-4">
            <div className="flex items-center justify-center gap-3">
              ...
              <span className="text-emerald-700 font-medium">Generating new AI analysis...</span>
              <span className="text-emerald-600 text-sm">Your current analysis is shown below</span>
            </div>
          </CardContent>
        </Card>
      )}

      {/* Error Banner (when refresh fails but we have existing analysis) */}
      {error && analysis && (
            ...
            <CardContent className="relative pt-8 pb-8">
              <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                  <p className="text-slate-400 text-sm font-medium mb-3">Overall Risk Score</p>
          ...
          <Card className="border-0 shadow-md bg-gradient-to-r from-amber-50 via-orange-50 to-amber-50">
            <CardContent className="py-6">
              <div className="flex gap-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-400 to-orange-400 flex items-center justify-center shadow-lg shadow-amber-500/20 shrink-0">
            ...
            <CardContent className="space-y-3">
              {displayAnalysis.riskFactors.map(risk => {
                ...
                return (
                  <div
                    key={risk.id}
                    className={`rounded-xl border transition-all ${isExpanded ? "border-emerald-200 bg-emerald-50/30" : "border-slate-100 bg-white hover:border-slate-200"}`}
                  >
                    <button
          ...
          {displayAnalysis.predictions && displayAnalysis.predictions.length > 0 && (
            <Card className="border-0 shadow-md">
              <CardHeader>
                ...
                <CardDescription>Potential outcomes if no changes are made</CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <CardContent>
                <div className="grid md:grid-cols-2 gap-3">
                  {displayAnalysis.lifestyleRecommendations.map((rec, i) => (
                    <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 hover:bg-emerald-50 transition-colors">
            ...
            <Card className="border-dashed">
              <CardContent className="py-16 text-center">
                ...
                <p className="text-slate-400 text-sm">
                  Your analysis history will appear here after generating new reports.
                </p>
              </CardContent>
            </Card>
          ) : (
