...
/** Public marketing count, keep this label uniform across Labs, membership, and promo. */
export const PUBLIC_BIOMARKER_COUNT_LABEL = "85+";
 ...
 */
const ESSENTIAL_COUNTS = { total: 92, measurable: 46, derived: 46 };
const ADVANCED_COUNTS = { total: 108, measurable: 60, derived: 48 };
const COMPLETE_COUNTS = { total: 118, measurable: 70, derived: 48 };
...
export interface BiomarkerSubscriptionPlan {
  ...
  /** When true, plan stays in catalog/billing but is hidden on public biomarkers pages. */
...
export const BIOMARKER_SUBSCRIPTION_PLANS: BiomarkerSubscriptionPlan[] = [
  {
    ...
    highlights: [
      ...
      "Portal calculates Superpower-style scores (HOMA-B, FIB-4, KDIGO, SII, PhenoAge)",
    ],
  },
  {
    id: "advanced",
  ...
  },
  {
    ...
    highlights: [
      "Full 118-marker catalog (tested + calculated)",
...
export function getBiomarkerSubscriptionPlan(
  ...
  return (
    ...
    BIOMARKER_SUBSCRIPTION_PLANS.find((p) => p.id === "essential")!
  );
}

export function getPublicBiomarkerSubscriptionPlans(): BiomarkerSubscriptionPlan[] {
  return BIOMARKER_SUBSCRIPTION_PLANS.filter((plan) => !plan.hiddenFromPublic);
}
