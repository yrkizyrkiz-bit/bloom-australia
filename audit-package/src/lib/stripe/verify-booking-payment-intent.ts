...
async function verifyPaymentIntentCore(
  ...
  if (params.consultProgram.isWeightManagement) {
    const normalizedPlan = normalizeWmSelectedPlan(params.selectedPlan);
    ...
    const metaProgram = (metadata.program || metadata.type || "")
      .replace(/_plan$/, "")
      .trim();
    if (metaProgram && metaProgram !== params.consultProgram.slug) {
  ...
  if (stripeCustomerId && paymentIntent.customer) {
    ...
    if (piCustomer !== stripeCustomerId) {
  ...
  return {
    ...
    normalizedPlan: params.consultProgram.isWeightManagement
      ? normalizeWmSelectedPlan(params.selectedPlan)
      : null,
...
async function resolveStripeCustomerIdForUser(
  ...
  const match = existing.data.find((c) => c.metadata?.userId === userId);
